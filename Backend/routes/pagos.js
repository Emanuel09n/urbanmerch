...
router.post('/crear-preferencia', async (req, res) => {
  const { productos, usuario, descuento, envio } = req.body
  ...
  try {
    ...
    const response = await preference.create({
      body: {
        ...
        notification_url: 'https://urbanmerch-production.up.railway.app/api/pagos/webhook',
        statement_descriptor: 'URBANMERCH',
        metadata: {
          email: usuario?.email,
          nombre: envio?.nombre || usuario?.nombre,
  ...
  if (type === 'payment') {
    try {
      ...
      if (pago.status === 'approved') {
        ...
        const envioNombre = metadata.envio_nombre || nombre
        const envioDireccion = metadata.envio_direccion || ''
        const envioCiudad = metadata.envio_ciudad || ''
        const envioTelefono = metadata.envio_telefono || ''
        ...
        if (email && productos.length > 0) {
          db.query('SELECT id FROM usuarios WHERE email = ?', [email], (err, users) => {
            if (!err && users.length > 0) {
              ...
              db.query(
                `INSERT INTO pedidos 
                (usuario_id, total, estado, nombre_envio, direccion, ciudad, telefono) 
                VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [usuario_id, total, 'pagado', envioNombre, envioDireccion, envioCiudad, envioTelefono],
                (err2, result) => {
                  if (!err2) {
                      ...
                      html: `
                        <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;background:#fff">
                          ...
                          <div style="padding:48px 40px">
                            ...
                            ${envioDireccion ? `
                            <div style="background:#f0f8ff;border:1px solid #cce4ff;border-radius:12px;padding:20px;margin-bottom:24px">
                              <p style="font-size:12px;color:#888;letter-spacing:2px;text-transform:uppercase;margin-bottom:12px">📍 Datos de envío</p>
                              <p style="font-size:14px;color:#333;margin-bottom:4px"><strong>${envioNombre}</strong></p>
                              <p style="font-size:14px;color:#666;margin-bottom:4px">${envioDireccion}</p>
                              <p style="font-size:14px;color:#666;margin-bottom:4px">${envioCiudad}</p>
                              <p style="font-size:14px;color:#666;">📞 ${envioTelefono}</p>
                            </div>
                            ` : ''}
