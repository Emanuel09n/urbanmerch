...
export default function Producto() {
  ...
  const handleAgregar = () => {
    if (!tallaSeleccionada) { alert('Por favor selecciona una talla'); return }
  ...
  const todasFotos = [producto.imagen, ...producto.fotos.map(f => f.url)].filter(Boolean)

  return (
    <div style={{ paddingTop: '70px', minHeight: '100vh', background: '#fff' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: isMobile ? '24px 16px' : '60px 40px' }}>
        ...
        <div style={{
          ...
          <div>
            <div style={{
              ...
              background: '#f8f8f8', marginBottom: '12px',
              height: isMobile ? '300px' : '520px',
              display: 'flex', alignItems: 'center', justifyContent: 'center'
            }}>
              {todasFotos.length > 0 ? (
                <img src={todasFotos[fotoActual]} alt={producto.nombre}
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                {todasFotos.map((foto, i) => (
                  <div key={i} onClick={() => setFotoActual(i)} style={{
                    ...
                    border: fotoActual === i ? '2px solid #000' : '2px solid transparent',
          ...
          <div>
            {producto.categoria && (
              <p style={{ fontSize: '11px', letterSpacing: '3px', textTransform: 'uppercase', color: '#999', marginBottom: '10px' }}>
                {producto.categoria}
              </p>
            )}
            ...
            <p style={{
              ...
              fontWeight: '900', letterSpacing: '-1px', marginBottom: '16px'
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '24px' }}>
              <FiPackage size={15} color="#888" />
              <span style={{ fontSize: '13px', color: '#888' }}>{producto.stock} unidades disponibles</span>
            </div>
            ...
            {producto.tallas.length > 0 && (
              <div style={{ marginBottom: '28px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                  <p style={{ fontSize: '12px', fontWeight: '700', letterSpacing: '1px', textTransform: 'uppercase' }}>
                    Talla {tallaSeleccionada && `— ${tallaSeleccionada}`}
                  </p>
                  <p style={{ fontSize: '12px', color: '#888', textDecoration: 'underline', cursor: 'pointer' }}>
                    Guía de tallas
                  </p>
                </div>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {producto.tallas.map(t => (
                    <button key={t.id} onClick={() => setTallaSeleccionada(t.talla)} style={{
                      width: isMobile ? '48px' : '56px',
                      height: isMobile ? '48px' : '56px',
                      borderRadius: '10px', fontSize: '13px', fontWeight: '600',
                      border: tallaSeleccionada === t.talla ? '2px solid #000' : '1.5px solid #e0e0e0',
                      ...
                      <span style={{
                        ...
                        fontSize: '9px', color: '#aaa', whiteSpace: 'nowrap'
                      }}>{t.stock} uds</span>
                    </button>
                  ))}
                </div>
            ...
            <button onClick={handleAgregar} style={{
              ...
              gap: '10px', transition: 'all 0.3s', marginTop: '36px'
            }}>
            ...
            <div style={{ marginTop: '32px', borderTop: '1px solid #f0f0f0', paddingTop: '24px' }}>
              {[
                ...
                <div key={i} style={{
                  ...
                  padding: '10px 0', borderBottom: '1px solid #f8f8f8', fontSize: '13px'
                }}>
